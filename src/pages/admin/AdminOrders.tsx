...
import { LayoutDashboard, Package, ShoppingBag, LogOut, Eye, X, LifeBuoy, Tag } from 'lucide-react'
                  ...
                  {orders.map(order => (
                    <tr key={order.id}>
                      ...
                      <td style={{ fontWeight: 600, fontSize: '0.875rem' }}>
                        ...
                        {order.product_details?.target_url && (
                          <div style={{ marginTop: 4 }}>
                            <a href={order.product_details.target_url} target="_blank" rel="noreferrer" style={{ fontSize: '0.75rem', color: 'var(--accent)', textDecoration: 'underline' }}>
                              View Link
                            </a>
                          </div>
                        )}
      ...
      {approvalModal.open && approvalModal.order && (
        <div className="modal-overlay" onClick={() => setApprovalModal({ open: false, order: null })}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            {['followers', 'likes', 'views'].includes(approvalModal.order.product_details?.category || '') ? (
              /* SOCIAL BOOST ORDER — Simple confirmation */
              <>
                <div style={{ background: 'rgba(124,58,237,0.1)', border: '1px solid var(--accent)', padding: 16, borderRadius: 10, marginBottom: 20 }}>
                  <p style={{ fontWeight: 700, marginBottom: 6 }}>{approvalModal.order.product_details?.category.toUpperCase()} Order Details:</p>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: 20 }}>
                  Clicking confirm will notify the buyer that their order is being processed and will be delivered within 24 hours.
                </p>
                <button 
