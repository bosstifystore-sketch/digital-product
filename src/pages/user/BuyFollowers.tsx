...
export default function BuyFollowers() {
  const { user } = useAuth()
  const [rates, setRates] = useState({ followers: 90, likes: 20, views: 5 })
  ...
  useEffect(() => {
    async function fetchRates() {
      ...
      if (data) {
        const newRates = { followers: 90, likes: 20, views: 5 }
        data.forEach(item => {
          if (item.key === 'follower_rate') newRates.followers = Number(item.value)
          if (item.key === 'like_rate') newRates.likes = Number(item.value)
          if (item.key === 'view_rate') newRates.views = Number(item.value)
        })
        setRates(newRates)
      }
      setLoading(false)
  ...
  const price = (quantity / 1000) * currentRate
  ...
  const handleBuy = () => {
    ...
    if (!url.includes('instagram.com/')) {
      setError(`Please enter a valid Instagram ${serviceType === 'followers' ? 'profile' : 'post'} URL.`)
      return
    }
    ...
    <div className="container" style={{ paddingTop: 40, paddingBottom: 60 }}>
      ...
      {checkoutOpen && (
        <CheckoutModal 
          ...
          amount={price}
          product={{ 
            id: `dynamic-${serviceType}-package`, 
            title: `${quantity.toLocaleString('en-IN')} Instagram ${serviceType.charAt(0).toUpperCase() + serviceType.slice(1)}`, 
            description: `Deliver to: ${url}`,
            price: price,
            category: serviceType,
            images: ['https://placehold.co/400x400/7c3aed/ffffff?text=Boost'],
            stock_status: true 
          }}
          userProfileUrl={url}
      ...
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <h1 style={{ fontSize: 'clamp(1.6rem, 6vw, 2.5rem)', fontWeight: 800, marginBottom: 10 }}>Boost Your Instagram</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 'clamp(0.9rem, 3vw, 1.1rem)' }}>
            Choose between followers, likes, or views for instant delivery.
          </p>
        </div>

        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginBottom: 24, flexWrap: 'wrap' }}>
          {['followers', 'likes', 'views'].map(type => (
            <button
              key={type}
              onClick={() => setServiceType(type as any)}
              className={`btn btn-sm ${serviceType === type ? 'btn-primary' : 'btn-secondary'}`}
              style={{ textTransform: 'capitalize', width: 120, padding: 10 }}
            >
              {type}
            </button>
          ))}
        ...
        <div className="card" style={{ padding: 'clamp(18px, 5vw, 32px)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ marginBottom: 32 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 20 }}>
              <div>
                <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'capitalize' }}>Select {serviceType}</h3>
          ...
          <div className="form-group" style={{ marginBottom: 24 }}>
            <label className="form-label">Target {serviceType === 'followers' ? 'Profile' : 'Post/Reel'} URL</label>
            <div style={{ position: 'relative' }}>
              ...
              <input 
                ...
                placeholder={`https://instagram.com/${serviceType === 'followers' ? 'your_username' : 'p/xyz...'}`}
                value={url}
          ...
          <div style={{ background: 'var(--bg-surface)', padding: 'clamp(14px, 4vw, 24px)', borderRadius: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 24, border: '1px solid var(--border)' }}>
            ...
            <div>
              ...
              <p style={{ color: 'var(--text-primary)', fontWeight: 600 }}>₹{currentRate} per 1K</p>
            </div>
          </div>
