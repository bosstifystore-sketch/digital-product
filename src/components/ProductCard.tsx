...
export default function ProductCard({ product }: Props) {
  ...
  const mainImage = validImages[0] || 'https://placehold.co/400x300/13131f/7c3aed?text=Boostify'
    ...
    <div className="card product-card" onClick={() => navigate(`/product/${product.id}`)}>
      <div className="product-card-img-wrap">
        <img
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = 'https://placehold.co/400x300/13131f/7c3aed?text=Boostify'
          }}
        />
        <span className={`product-card-badge ${product.stock_status ? 'badge-instock' : 'badge-outstock'}`}>
          {product.stock_status ? '✓ In Stock' : '✗ Sold Out'}
        </span>
      </div>
